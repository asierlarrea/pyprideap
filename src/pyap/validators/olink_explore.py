...
class OlinkExploreValidator:
    ...
    def _check_qc_consistency(self, ds: AffinityDataset) -> list[ValidationResult]:
        if "SampleQC" not in ds.samples.columns:
            return []
        results = []
        for i, row in ds.samples.iterrows():
            qc = row.get("SampleQC")
            if i >= len(ds.expression):
                continue
            expr_row = ds.expression.iloc[i]
            if qc in ("FAIL", "NA"):
                if not expr_row.isna().all():
                    results.append(ValidationResult(
                        level=Level.ERROR,
                        rule="olink.npx.qc_consistency",
                        message=f"Sample {row.get('SampleID', i)}: SampleQC={qc} but NPX contains non-NaN values",
                        details={"sample_index": int(i), "qc": qc},
                    ))
                ...
                if expr_row.isna().all():
                    results.append(ValidationResult(
                        level=Level.ERROR,
                        rule="olink.npx.qc_consistency",
                        message=f"Sample {row.get('SampleID', i)}: SampleQC={qc} but all NPX values are NaN",
                        details={"sample_index": int(i), "qc": qc},
                    ))
        return results
